...
// Route handler instead of a page: a page can't return a real 404 here —
// it suspends on the DB lookup, Next commits the 200 shell before
// notFound() fires (streaming), and crawlers index bogus slugs. A route
// handler sets the status explicitly. The page was already a self-contained
...
<title>Not Found | Novyx Vault</title>
<style>${STYLES}</style>
</head>
<body>
<article>
<h1>Note not found</h1>
<p>This note doesn't exist or is no longer published.</p>
</article>
<div class="footer"><a href="https://vault.novyxlabs.com">Novyx Vault</a></div>
</body>
</html>`,
      404
    );
  ...
  const url = `https://vault.novyxlabs.com/p/${esc(slug)}`;
  const dateHtml = note.published_at
    ? `<p class="date">${new Date(note.published_at).toLocaleDateString("en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })}</p>`
...
${note.published_at ? `<meta property="article:published_time" content="${esc(note.published_at)}">` : ""}
...
<div class="footer">
Written in <a href="https://vault.novyxlabs.com" target="_blank" rel="noopener">Novyx Vault</a>
</div>
</body>
</html>`,
    200
