...
export default function Home() {
  ...
  const handleNavigateWikiLink = useCallback(
    async (linkText: string) => {
      ...
      if (resolved) {
        ...
        const shouldCreate = confirm(`Note "${linkText}" not found. Create it?`);
        if (shouldCreate) {
          try {
            await fetch("/api/notes", {
              method: "POST",
  ...
  const handleCreateNoteFromChat = useCallback(async (aiContent: string) => {
    const firstLine = aiContent.split("\n")[0];
    const defaultTitle = firstLine.replace(/^#+\s*/, "").trim().substring(0, 50);
    const name = prompt("Note name:", defaultTitle);
    if (!name) return;
