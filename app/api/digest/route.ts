...
// POST — send daily digest email for a user (called by cron or manually)
// Also handles opt-in/out via PUT
export async function POST(req: NextRequest) {
  try {
    ...
    // Get users who have opted in to digest
    let users: { id: string; email: string; settings: Record<string, unknown> }[];
    ...
    if (userId) {
      ...
      users = [{ id: data.id, email: authUser.user.email, settings: (data.settings || {}) as Record<string, unknown> }];
    } else {
      // Batch: get all users with digest enabled
      const { data } = await supabase
        .from("profiles")
        .select("id, settings")
      ...
      const userPromises = data.map(async (profile) => {
        ...
        return authUser.user?.email
          ? { id: profile.id, email: authUser.user.email, settings: (profile.settings || {}) as Record<string, unknown> }
          : null;
      });
      users = (await Promise.all(userPromises)).filter(Boolean) as typeof users;
    }
